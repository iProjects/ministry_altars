...
namespace ministry_altars.Data
{
    ...
    public sealed class mssqlapisingleton
    {
        ...
        private string CONNECTION_STRING = @"Data Source=.\SQLEXPRESS;Database=ministry_altars_database;User Id=sa;Password=123456789";
        ...
        public bool createdatabase()
        {
            try
            {
                string _default_database_name = utilzsingleton.getInstance(_notificationmessageEventname).getappsettinggivenkey("mssql_database", "ministry_altars_database");
        ...
        public responsedto create_tables(mssqlconnectionstringdto _connectionstringdto)
        {
            ...
            try
            {
                ...
                //Create altars table 
                string SQL_CREATE_ALTARS_TABLE = "IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[" + DBContract.altars_entity_table.TABLE_NAME + "]') AND type in (N'U'))";
                SQL_CREATE_ALTARS_TABLE += " BEGIN ";
                SQL_CREATE_ALTARS_TABLE += " CREATE TABLE " + DBContract.altars_entity_table.TABLE_NAME + " (" +
                      DBContract.altars_entity_table.ID + " INT IDENTITY(1,1) PRIMARY KEY NOT NULL, " +
                ...
                string SQL_CREATE_PASTORS_TABLE = "IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[" + DBContract.pastors_entity_table.TABLE_NAME + "]') AND type in (N'U'))";
                SQL_CREATE_PASTORS_TABLE += " BEGIN ";
                SQL_CREATE_PASTORS_TABLE += " CREATE TABLE " + DBContract.pastors_entity_table.TABLE_NAME + " (" +
                      DBContract.pastors_entity_table.ID + " INT IDENTITY(1,1) PRIMARY KEY NOT NULL, " +
        ...
        public responsedto checkconnectionasadmin(mssqlconnectionstringdto _connectionstringdto)
        {
            ...
            try
            {
                ...
                {
                    _responsedto.isresponseresultsuccessful = true;
                    _responsedto.responsesuccessmessage = "connection to mssql successfull. altar count [ " + count + " ]";
                    return _responsedto;
                }
                else
                {
